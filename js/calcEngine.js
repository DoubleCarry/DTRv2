/**
 * DTR Calculation Engine (Client-side mirror)
 * Guarantees 100% parity with backend calculation logic.
 */
...
export function fmtHours(hours) {
  return fmtHM(Math.round((hours || 0) * 60));
}
...
export function calcEntryCreditedDuration(entry, userSettings = {}) {
  ...
  if (useFixedSchedule && !earlyArrivalCountsAsOvertime) {
    ...
    if (inM < schedStartM && inM >= (schedStartM - 360)) {
...
export function predictCompletion(records = [], targetHours = 300, userSettings = {}, customHolidays = []) {
  ...
  return {
    ...
    isCompleted: false,
    date: predictedDate,
    dateLabel: fmtDate(predictedDate),
